...
class SegmentationLosses_oc(CrossEntropyLoss):
    ...
    def __init__(self, se_loss=False, se_weight=0.2, nclass=-1,
                 aux=False, aux_weight=0.4, weight=None,
                 size_average=True, ignore_index=-1, reduction='mean'):
        super(SegmentationLosses_oc, self).__init__(weight, ignore_index=ignore_index, reduction=reduction)
    ...
    def forward(self, *inputs):
        if not self.se_loss and not self.aux:
            return super(SegmentationLosses_oc, self).forward(*inputs)
        elif not self.se_loss:
            # *preds, target = tuple(inputs)
            # pred1, pred2, pred3 = tuple(preds[0])
            pred1, pred2, pred3, pred4, target = tuple(inputs)
            loss1 = super(SegmentationLosses_oc, self).forward(pred1, target)
            ...
            pred, se_pred, target = tuple(inputs)
            se_target = self._get_batch_label_vector(target, nclass=self.nclass).type_as(pred)
            loss1 = super(SegmentationLosses_oc, self).forward(pred, target)
            loss2 = self.bceloss(torch.sigmoid(se_pred), se_target)
            return loss1 + self.se_weight * loss2
        else:
            pred1, se_pred, pred2, target = tuple(inputs)
            se_target = self._get_batch_label_vector(target, nclass=self.nclass).type_as(pred1)
            loss1 = super(SegmentationLosses_oc, self).forward(pred1, target)
            loss2 = super(SegmentationLosses_oc, self).forward(pred2, target)
            loss3 = self.bceloss(torch.sigmoid(se_pred), se_target)
            return loss1 + self.aux_weight * loss2 + self.se_weight * loss3
